...
from segmentation_heads.upernet.model import CARL_Adapter


class ViTAdapterTrainer(pl.LightningModule):
    ...
    def __init__(self, config: Dict[str, Any], *args, **kwargs):
        ...
        self.model = CARL_Adapter(**model_kwargs)
        if self.config_accessor.get("training_kwargs/freeze_spat_encoder", True):
            self._freeze_parameters(self.model.spatial_encoder.spatial_backbone)
        if self.config_accessor.get("training_kwargs/freeze_spec_encoder", True):
    ...
    def training_step(
        ...
        cnn_image = img.mean(dim=1, keepdim=True)

        # Forward pass through CARL model
        predictions = self.model(img, wavelengths, cnn_image)
