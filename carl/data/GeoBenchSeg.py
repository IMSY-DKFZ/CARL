...
class GeoBenchSeg(Dataset):
    """GeoBench semantic segmentation dataset.
    
    Loads spectral imagery from GeoBench with associated wavelength information
    and semantic segmentation labels. Implements lazy loading with caching.
    ...
    def _extract_and_validate_label(self, sample: Any) -> np.ndarray:
        """Extract and validate semantic labels.
        ...
        """
        if not hasattr(sample.label, 'data'):
            raise AttributeError("Label does not have 'data' attribute")
